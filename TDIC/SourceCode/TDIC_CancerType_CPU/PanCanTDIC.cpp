...
void PanCanTDIC(PanCanGTMatrix& gtMatrix, TDIMatrix& geMatrix, map<string, 
        string>& mapGlobDrivers, const int tumorID, const string outPath, const float v0, int numCanType){
    // initializations 
    ...
    float* tumorPosteriorMatrixC = new float[nGT * nGE * numCanType]();//save each cancer type tumorPosteriorMatrix
