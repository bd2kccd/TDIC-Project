...
//    string gtFilePath, gtcFilePath, globalDriverPath, degFilePath, outPath;
    string gtFilePath, gtcFilePath, degFilePath, outPath,strv0;

    while((hasOpt = getopt(argc, argv, "h:f:d:v:o:c:?")) != -1)
    {
        switch(hasOpt)
        {
            case 'f':
            ...
            case 'h':
                cerr << "Usage: TDIC -f inputGaMatrix -d inputGeMatrix -o pathForOutputResults [ -v0 constant v0]\n";
                exit(1);
                break;
            ...
            default:
                cerr << "Usage: TDIC -f inputGaMatrix -d inputGeMatrix -v constant v0 -o pathForOutputResults\n";
                abort();
        }
    }
