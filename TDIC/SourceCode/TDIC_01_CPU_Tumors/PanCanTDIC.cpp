...
void PanCanTDIC(PanCanGTMatrix& gtMatrix, TDIMatrix& geMatrix, map<string, 
        string>& mapGlobDrivers, vector<int>  tumorGtIndices, vector<int> tumorGeIndices, const float v0, vector<float>& tumorPosteriorMatrix, int tumorCanType){
   ...
   //calculate lntumorPriors
    ...
    vector<float> tumorPosteriorMatrixC(nGT * nGE * 2, 0.0);//save cancer type the same(1) or not(0) tumorPosteriorMatrix
