...
//        int  *d_gtDataMatrix, *d_geDataMatrix;
...
//        cudaMalloc( (int**)&d_gtDataMatrix,numColT*numRowT*sizeof(int) );
        cudaMalloc( (bool**)&d_gtDataMatrix,numColT*numRowT*sizeof(bool) );
        
        int numColE = geMatrix->nCol;
        int numRowE = geMatrix->nRow;
//        cudaMalloc( (int**)&d_geDataMatrix,numColE*numRowE*sizeof(int) );
        cudaMalloc( (bool**)&d_geDataMatrix,numColE*numRowE*sizeof(bool) );
...
//        #pragma omp parallel for
    ...
    else//process PanCanGTMatrix
    {
        ...
        //prepare non-tumor specific device variable for GPU kernel invoke
        //define the host variables
//        int* gtDataMatrix = panCanGtMatrix->getMatPtr();
//        int* geDataMatrix = geMatrix->getMatPtr();
        bool* gtDataMatrix = panCanGtMatrix->getMatPtr();
        bool* geDataMatrix = geMatrix->getMatPtr();
        vector<int> cancerTypes = panCanGtMatrix-> getCanTypes();
        //Get number of cancer types. Use max value of the cancer type as the number of cancer types
        int numCanTypes = *max_element(cancerTypes.begin(), cancerTypes.end());
...
//        cudaMemcpy(d_geDataMatrix, geDataMatrix, numColE*numRowE*sizeof(int), cudaMemcpyHostToDevice);
        ...
        //set cuda device heap memory to 10M. default is 8M, not enough in PanCanTDIC_GeLoop_Cuda when using dynamic array in device code
