...
void TDIC(GTMatrix& gtMatrix, TDIMatrix& geMatrix, map<string, 
        string>& mapGlobDrivers, const int tumorID, const string outPath, const float v0, bool *&d_gtDataMatrix, bool *&d_geDataMatrix){    
    // initializations 
...
//    cudaMemcpy(d_geDataMatrix, geDataMatrix, numColE*numRowE*sizeof(int), cudaMemcpyHostToDevice);    
    cudaMemcpy(d_tumorPosteriorMatrix, &tumorPosteriorMatrix[0], nGT*nGE*sizeof(float), cudaMemcpyHostToDevice);
    ...
    TDIC_GeLoop_Cuda<<<grid, block>>>(nGE, nGT, nTumors, d_tumorGeIndices, d_tumorGtIndices, d_tumorGlobDriverIndices, d_gtDataMatrix, d_geDataMatrix, d_lntumorMutPriors ,d_tumorPosteriorMatrix);
