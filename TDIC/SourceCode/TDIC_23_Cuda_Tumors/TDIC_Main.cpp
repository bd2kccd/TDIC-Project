...
int main(int argc, char** argv) {
    ...
    while((hasOpt = getopt(argc, argv, "hi:f:d:g:o:c:s:e:?")) != -1)
    {
        switch(hasOpt)
        {
            case 'i':
    ...
    TDIC_Load(inputDataFile, gtGeneNames, geGeneNames, tumorNames);
    ...
    //showing processing info on screen. 
        ...
        */
        ...
        if (!gtcFilePath.empty())
        {
            PanCanTDIC(*panCanGtMatrix, *geMatrix, globalDriverMap, tumorGtIndices, tumorGeIndices, v0, tumorPosteriorMatrix, numCanTypes, d_cancerTypes, d_gtDataMatrix, d_geDataMatrix);
        }
        else
