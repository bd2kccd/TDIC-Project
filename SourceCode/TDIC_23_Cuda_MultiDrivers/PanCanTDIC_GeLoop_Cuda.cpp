 ...
 */     
__global__ void PanCanTDIC_GeLoop(int nGE, int nGT, int nTumors, int realNumOfCanTypes, int* tumorGeIndices,  int* tumorGtIndices, int* tumorGlobDriverIndices, int d_maxDriverSize, int* canTypes, bool* gtDataMatrix, bool* geDataMatrix, float* lntumorMutPriors, float* tumorPosteriorMatrix, float* tumorPosteriorMatrixC)
{
...
//        int rowStartForGlobDriver = curGDriverIndx * nTumors;
        ...
        //Suppose we only have three global drivers for each ge, so d_maxDriverSize=3
        int rowStartForGlobDriver[3] = {0};
        for (int i=0; i<3; i++)
            rowStartForGlobDriver[i] = tumorGlobDriverIndices[ge*3+i] * nTumors;
        ...
        for (int gt = 0; gt < nGT; gt++)
        {
            ...
            float CT[2*nCanType] = {0.0};
            float CTE[4*nCanType] = {0.0};
            float CTD[4*nCanType] = {0.0};
            float CTDE[8*nCanType] = {0.0}; 
...
//                    rowStartForGD = gdIndex * nTumors;
//                    dVal = dVal || gtDataMatrix[rowStartForGD + t];
//                    if (dVal == 1)
//                        break;
//                }
                for (int i=0; i<3; i++)
                    dVal = dVal || gtDataMatrix[rowStartForGlobDriver[i] + t];
                
                //Only need to count CTDE, all other variables can be calculated from CTDE
                
                //currentTumorCanType is from 1 to realNumOfCanTypes, the array starts from 0, so needs to minus 1
                CTDE[(tumorCanType-1)*8+tVal*4+dVal*2+eVal]++;
            }
            
            //Calculate CTE,CTD from CTDE
            for(int a=0; a<realNumOfCanTypes; a++)
                for(int b=0; b<=1; b++)
                    for(int c=0; c<=1; c++)
                    {
            ...
            for(int a=0; a<=1; a++)
                for(int b=0; b<=1; b++)
                    for(int c=0; c<=1; c++)
                          for (int i=0; i<realNumOfCanTypes; i++) //there are realNumOfCanTypes cancer types
            ...
            //Calculate C[] from CT
            for (int i=0; i<realNumOfCanTypes; i++)
            ...
            */
            ...
            float lnDataC[nCanType] = {0.0};
            
            float TFscoreC[nCanType] = {0.0};
            float DFscoreC[nCanType] = {0.0};
            float pGT1GE1C[nCanType] = {0.0};
            float pGT0GE1C[nCanType] = {0.0};             
            //float lnDataC1 = 0.0;
        ...
        float normalizerC[nCanType] = {0.0};
        ...
        float normC[nCanType] = {0.0}; //for each cancer type
