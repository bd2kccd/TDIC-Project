...
void PanCanTDIC(PanCanGTMatrix& gtMatrix, TDIMatrix& geMatrix, map<string, 
        vector<string> >& mapGlobDrivers, const int tumorID, const string outPath, const float v0,  int realNumOfCanTypes, int  *&d_cancerTypes,  bool *&d_gtDataMatrix, bool *&d_geDataMatrix){
    // initializations 
...
//    int maxVecSize = 0;
    ...
    if (numOfVec != nGE)
    {
        cerr << "Error: The size of tumorGlobDriverIndices is not the same with the size of Ge indices which should be. ";
        exit(1);
    }
    //Get maVecSize
...
//    }
    //Transfer 2Dim vector into one dim vector
...
//    delete[] arr_tumorGlobDriverIndices;
    ...
    PanCanTDIC_GeLoop<<<grid, block>>>(nGE, nGT, nTumors, realNumOfCanTypes, d_tumorGeIndices, d_tumorGtIndices, d_tumorGlobDriverIndices, maxVecSize,  d_cancerTypes, d_gtDataMatrix, d_geDataMatrix, d_lntumorMutPriors ,d_tumorPosteriorMatrix, d_tumorPosteriorMatrixC);
