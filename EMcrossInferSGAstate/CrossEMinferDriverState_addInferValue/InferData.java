...
public class InferData {
    ...
    private ArrayList<ArrayList<Double>> driverSGATable ;//driverSGATable save inferActivation as well
...
//                    String strDEG = items[2].trim();
    ...
    public void readInGtMatrix(String fileGtMatrix) {
        ...
        driverSGATable = new ArrayList<ArrayList<Double>>();
        ...
        for (int i = 0; i < tumorSize; i++) {
            ArrayList<Double> SGAValueRow = new ArrayList<Double>();
            for (int j = 0; j < SGASize; j++) {
                double SGAvalue = driverSGATable_ori.get(i).get(j); 
                SGAValueRow.add(SGAvalue);
            }  
...
//        }
    ...
    public void estimateParams_firsttime() {
        //count original SGA, DEG value to get the edge parameter
        ...
        for (String strEdge : solidEdgeList) {
            ...
            for (int t = 0; t < driverSGATable.size(); t++) {
                int SGAValue = driverSGATable.get(t).get(SGAIndx).intValue();
...
//                    SGACount[SGAValue] += 1;//If table has missing value then this count is not appropriate
    ...
    public void estimateParams_afterfirsttime() {
        //add infervalue of last state to get edge parameter
        mapEdgeParam = new HashMap<String, Double[]>();
        mapSGAParam = new HashMap<String, Double[]>();
        mapSGAParam.clear();
        ...
        Double[] edgeCount = new Double[]{0.00001,0.00001,0.00001,0.00001};//initialized with 0.00001 to avoid divided by 0 in calculating probability
        Double[] SGACount = new Double[]{0.00001,0.00001};//initialized with 1 to avoid divided by 0 in calculating probability
        String[] items;
        for (String strEdge : solidEdgeList) {
            items = strEdge.split(",");
            String SGA = items[0];
            ...
            int SGAIndx = mapSGA.get(SGA);
            int DEGIndx = mapDEG.get(DEG);
            Arrays.fill(edgeCount, 0.00001);//re-initialization
            Arrays.fill(SGACount, 0.00001);
            //Need to check if current SGA has been counted before
            boolean SGACounted = false;
            if (mapSGAParam.containsKey(SGA)) {
                SGACounted = true;
            ...
            for (int t = 0; t < driverSGATable.size(); t++) {
                double SGAinferValue = driverSGATable.get(t).get(SGAIndx);
                int DEGValue = targetDEGTable.get(t).get(DEGIndx);
//                edgeCount[SGAValue * 2 + DEGValue] += 1;//If table has missing value then this count is not appropriate
                if (DEGValue == 1){
                    edgeCount[3] += SGAinferValue;
                    edgeCount[1] += 1 - SGAinferValue;
                }
                else{ 
                    edgeCount[2] += SGAinferValue;
                    edgeCount[0] += 1 - SGAinferValue;
                }
                
                if (!SGACounted) {
                    SGACount[1] += SGAinferValue;
                    SGACount[0] += 1 - SGAinferValue;
                }
            }
            
            //normalize value to propability
            Double[] paramOfEdge = new Double[4];
            Double[] paramOfSGA = new Double[2];
    ...
    public void inferDriverActivation(){
        ...
        for (int i = 0; i < tumorSize; i++) {
            ...
            for (int j = 0; j < SGASize; j++) {
                ...
                if (driverSGATable_ori.get(i).get(j).intValue() == 1) {
    ...
    public void inferDriverActivation_testingSet(){
        //differ with inferDriverActivation at: do not consider SGA = 1 value
        System.out.println("Infer driver activation...");
        driverActivationTable = new ArrayList<ArrayList<Double>>();
...
//    }
    ...
    public double compareMatrix() {
        ...
        for (int i = 0; i < tumorSize; i++) {
            for (int j = 0; j < SGASize; j++) {
                double vNew = driverActivationTable.get(i).get(j);
                double vOld = driverSGATable.get(i).get(j);
                diff += pow((vNew - vOld),2);
            }
        }
        double aveDiff = diff/totalNum;
        return aveDiff;
    }
    ...
    public void updateDriverSGATable() {
        ...
        //copy driverActivationTable to driverSGATable
        int tumorSize = driverSGATable.size();
        int SGASize = driverSGATable.get(0).size();
        for (int i = 0; i < tumorSize; i++) {
            for (int j = 0; j < SGASize; j++) {
                double v = driverActivationTable.get(i).get(j);
