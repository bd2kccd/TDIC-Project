...
import tetrad.data.*;
import tetrad.graph.*;
import tetrad.util.ChoiceGenerator;
import tetrad.util.ForkJoinPoolInstance;
import tetrad.util.TaskManager;
import tetrad.util.TetradLogger;
...
//      private Logger logger = Logger.getLogger("Fgs");
    /**
...
//        this.logger.info("Returning this graph: " );
...
//        this.logger.info("** FORWARD EQUIVALENCE SEARCH");
...
//        this.logger.info("** BACKWARD EQUIVALENCE SEARCH");
        sortedArrows = new ConcurrentSkipListSet<>();
        lookupArrows = new ConcurrentHashMap<>();
        neighbors = new ConcurrentHashMap<>();
...
//                }
    ...
    private boolean insert(Node x, Node y, Set<Node> T, double bump) {
        ...
        if (verbose) {
            ...
            TetradLogger.getInstance().log("insertedEdge??s", graph.getNumEdges() + ". INSERT " + graph.getEdge(x, y) +
                    " " + T + " " + bump + " " + label);
//            this.logger.info("insertedEdges " + graph.getNumEdges() + ". INSERT " + graph.getEdge(x, y) +
//                    " " + T + " " + bump + " " + label);
        }
...
//                this.logger.info("directedEdges:"+ message);
                out.println(message);
            }
        }
...
//            this.logger.info("deletedEdges:"+ message);
            out.println(message);
        }
...
//                this.logger.info("insertedEdges:"+ "Adding edge by knowledge: " + graph.getEdge(nodeA, nodeB));
            }
        }
        for (Edge edge : graph.getEdges()) {
...
//                        this.logger.info("insertedEdges:"+ "Adding edge by knowledge: " + graph.getEdge(nodeB, nodeA));
                    }
                }
