...
//        return (node1 == node2) || isProperDescendentOf(node1, node2);
    ...
    public synchronized boolean addEdge(Edge edge) {
        ...
        if (isAdjacentTo(edge.getNode1(), edge.getNode2())) {
            throw new IllegalArgumentException("Already adjacent." + edge.toString() );
        }
    ...
    //Added for TDI graph on 01/11/17 by mxj
  public Node getNode(String name, String type) {
        Node node = namesHash.get(name);
        if (node == null)
            return null;
        if (!node.getGeneType().equals(type)) {
            for (Node _node : nodes) {
              if (_node.getGeneType().equals(type) && _node.getName().equals(name)){
                  return _node;
              }
            }
            return null;
